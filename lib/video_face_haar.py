...
class FaceDetect():
    ...
    def update_average(self, x, y, w, h, reset=False):
        if reset:
            self.count = 1
            self.sumx = x
            self.sumy = y
    ...
    def find_face(self, raw_frame, time):
        ...
        biggest = 0
        avg_size = self.w * self.h
        raw_frame = cv2.rectangle(np.array(raw_frame), (int(self.x),int(self.y)), (int(self.x+self.w),int(self.y+self.h)), (255,255,255), 2)
